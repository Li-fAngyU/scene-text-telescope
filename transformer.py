...
class PositionalEncoding(nn.Layer):
    def __init__(self, d_model, dropout, max_len = 5000):
        ...
        div_term = paddle.exp(paddle.arange(0, d_model, 2, dtype=paddle.float32) * 
                             -(math.log(10000.0) / d_model))
        pe[:, 0::2] = paddle.sin(position * div_term)
        pe[:, 1::2] = paddle.cos(position * div_term)
        pe = pe.unsqueeze(0)
        self.register_buffer('pe', pe)
        # here can change learining pe!!!
...
class ResNet(nn.Layer):
    def __init__(self, num_in, block, layers):
        ...
        self.bn1 = nn.BatchNorm2D(64, momentum=0.1, use_global_stats=True)
        self.relu1 = nn.ReLU()
        self.pool = nn.MaxPool2D((2, 2), (2, 2))

        self.conv2 = nn.Conv2D(64, 128, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2D(128, momentum=0.1, use_global_stats=True)
        self.relu2 = nn.ReLU()
        ...
        self.layer1_bn = nn.BatchNorm2D(256, momentum=0.1, use_global_stats=True)
        ...
        self.layer2_bn = nn.BatchNorm2D(256, momentum=0.1, use_global_stats=True)
        ...
        self.layer3_bn = nn.BatchNorm2D(512, momentum=0.1, use_global_stats=True)
        ...
        self.layer4_conv2_bn = nn.BatchNorm2D(1024, momentum=0.1, use_global_stats=True)
    ...
    def _make_layer(self, block, inplanes, planes, blocks):
        ...
        if inplanes != planes:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2D(planes, momentum=0.1, use_global_stats=True))
        else:
            downsample = None
        layers = []
...
class BasicBlock(nn.Layer):
    def __init__(self, inplanes, planes, downsample):
        ...
        self.bn1 = nn.BatchNorm2D(planes, momentum=0.1, use_global_stats=True)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2D(planes, planes, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2D(planes, momentum=0.1, use_global_stats=True)
...
#from tbsrn import MultiHeadedAttention, LayerNorm, PositionwiseFeedForward
...
def attention(query, key, value, mask=None, dropout=None, attention_map=None):
    ...
    p_attn = paddle.nn.functional.softmax(scores, axis=-1)
...
class Transformer(nn.Layer):
    ...
    def __init__(self):
        ...
        #     if p.dim() > 1:
        #         nn.init.xavier_uniform_(p)  模型参数初始化未对齐，也不用对齐
    ...
    def forward(self, image, text_length, text_input, test=False, attention_map=None):
        ...
        probs_res = paddle.zeros([total_length, get_alphabet_len()],dtype=word_decoder_result.dtype)
